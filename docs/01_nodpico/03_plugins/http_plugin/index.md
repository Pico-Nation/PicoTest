...
The `http_plugin` is a core plugin supported by both `nodpico` and `kpicod`. The plugin is required to enable any RPC API functionality provided by a `nodpico` or `kpicod` instance.
...
```sh
# command-line
nodpico ... --plugin picoio::http_plugin [options]
 (or)
kpicod ... --plugin picoio::http_plugin [options]
```
...
Config Options for picoio::http_plugin:
  --unix-socket-path arg                The filename (relative to data-dir) to 
                                        ...
                                        blank to disable (=kpicod.sock for kpicod)
  --http-server-address arg (=127.0.0.1:8888 for nodpico)
