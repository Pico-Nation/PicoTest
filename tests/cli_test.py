...
def nodpico_help_test():
    """Test that nodpico help contains option descriptions"""
    help_text = subprocess.check_output(["./programs/nodpico/nodpico", "--help"])

    assert(re.search(b'Application.*Options', help_text))
    assert(re.search(b'Options for .*_plugin', help_text))


def clpico_help_test(args):
    """Test that clpico help contains option and subcommand descriptions"""
    help_text = subprocess.check_output(["./programs/clpico/clpico"] + args)
...
def cli11_bugfix_test():
    ...
    completed_process = subprocess.run(
        ['./programs/clpico/clpico', '-u', 'http://localhost:0/',
    ...
    assert(b'Failed to connect to nodpico' in completed_process.stderr)
...
def cli11_optional_option_arg_test():
    ...
    output = subprocess.check_output(['./programs/clpico/clpico', 'sign',
                                      '-c', chain, '-k', '{}'],
                                     input=key.encode(),
...
clpico_help_test(['--help'])
clpico_help_test(['system', '--help'])
clpico_help_test(['version', '--help'])
clpico_help_test(['wallet', '--help'])
