...
###############################################################
# nodpico_multiple_version_protocol_feature_test
#
# Test for verifying that older versions of nodpico can work with newer versions of nodpico.
#
###############################################################
...
def restartNode(node: Node, nodeId, chainArg=None, addSwapFlags=None, nodpicoPath=None):
    if not node.killed:
        node.kill(signal.SIGTERM)
    isRelaunchSuccess = node.relaunch(nodeId, chainArg, addSwapFlags=addSwapFlags,
                                      timeout=5, cachePopen=True, nodpicoPath=nodpicoPath)
...
try:
    ...
    # Finally, when we restart the 4th node with the version of nodpico that supports protocol feature,
    ...
    oldNode.relaunch(oldNodeId, chainArg="--import-reversible-blocks {}".format(portableRevBlkPath), timeout=1, nodpicoPath="programs/nodpico/nodpico")
    os.remove(portableRevBlkPath)

    restartNode(oldNode, oldNodeId, chainArg="--replay", nodpicoPath="programs/nodpico/nodpico")
