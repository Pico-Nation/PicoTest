...
PICOIO_STUFF_DIR=$(mktemp -d)
trap "rm -rf $PICOIO_STUFF_DIR" EXIT
NODPICO_LAUNCH_PARAMS="./programs/nodpico/nodpico -d $PICOIO_STUFF_DIR --config-dir $PICOIO_STUFF_DIR \
--chain-state-db-size-mb 8 --chain-state-db-guard-size-mb 0 --reversible-blocks-db-size-mb 1 \
--reversible-blocks-db-guard-size-mb 0 -e -ppicoio"

run_nodpico() {
   if (( $VERBOSE == 0 )); then
      $NODPICO_LAUNCH_PARAMS --http-server-address '' --p2p-listen-endpoint '' "$@" 2>/dev/null &
   else
      $NODPICO_LAUNCH_PARAMS --http-server-address '' --p2p-listen-endpoint '' "$@" &
   fi
}
...
run_expect_failure() {
   ...
   trap "kill $NODPICO_PID; wait $NODPICO_PID; exit 1" ALRM
   sleep 10
   if wait $NODPICO_PID; then exit 1; fi
   kill $TIMER_PID
   trap ALRM
}
