...
[[info]]
| Be aware that if a clear text `data` field is used, clpico need to fetch copies of required ABIs using `nodpico` API. That operation has a performance overhead on `nodpico`
...
```sh
clpico push transaction TRX_FILE.json
```

* Submit a transaction from a JSON:

```sh
clpico push transaction JSON
```
