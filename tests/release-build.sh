...
echo ''
echo '    The purpose of this test is to ensure that nodpico was built with compiler'
echo 'optimizations enabled. While there is no way to programmatically determine that'
echo 'given one binary, we do set a debug flag in nodpico when it is built with'
echo 'asserts. This test checks that debug flag. Anyone intending to build and install'
echo 'nodpico from source should perform a "release build" which excludes asserts and'
...
# find nodpico
[[ $(git --version) ]] && cd "$(git rev-parse --show-toplevel)/build/programs/nodpico" || cd "$(dirname "${BASH_SOURCE[0]}")/../programs/nodpico"
if [[ ! -f nodpico ]]; then
    echo 'ERROR: nodpico binary not found!'
    echo ''
    echo 'I looked here...'
    echo "$ ls -la \"$(pwd)/programs/nodpico\""
    ls -la "$(pwd)/programs/nodpico"
...
# run nodpico to generate state files
./nodpico --extract-build-info build-info.json 1>/dev/null 2>/dev/null
