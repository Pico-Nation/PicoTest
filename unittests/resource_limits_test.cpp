...
#include <picoio/testing/chainbase_fixture.hpp>

#include <boost/test/unit_test.hpp>

using namespace picoio::chain::resource_limits;
...
BOOST_AUTO_TEST_SUITE(resource_limits_test)
   ...
   BOOST_FIXTURE_TEST_CASE(elastic_cpu_relax_contract, resource_limits_fixture) try {
      ...
      const uint64_t expected_contract_iterations =
              expected_exponential_average_iterations(0, PICO_PERCENT(config::default_max_block_cpu_usage, config::default_target_block_cpu_usage_pct), config::default_max_block_cpu_usage, config::block_cpu_usage_average_window_ms / config::block_interval_ms ) +
   ...
   BOOST_FIXTURE_TEST_CASE(elastic_net_relax_contract, resource_limits_fixture) try {
      ...
      // subtracting 1 for the iteration that pulls double duty as reaching/exceeding the target and starting congestion handling
      const uint64_t expected_contract_iterations =
              expected_exponential_average_iterations(0, PICO_PERCENT(config::default_max_block_net_usage, config::default_target_block_net_usage_pct), config::default_max_block_net_usage, config::block_size_average_window_ms / config::block_interval_ms ) +
