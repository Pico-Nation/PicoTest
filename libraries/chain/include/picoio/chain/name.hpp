...
namespace fc {
  class variant;
  void to_variant(const picoio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, picoio::chain::name& check);
} // fc
...
namespace picoio::chain {
   ...
   private:
      ...
      friend void fc::from_variant(const fc::variant& v, picoio::chain::name& check);
...
#define N(X) picoio::chain::string_to_name(#X)

} // picoio::chain

namespace std {
   template<> struct hash<picoio::chain::name> : private hash<uint64_t> {
      typedef picoio::chain::name argument_type;
...
FC_REFLECT( picoio::chain::name, (value) )
