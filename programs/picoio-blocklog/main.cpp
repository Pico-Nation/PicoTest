...
struct report_time {
    ...
    void report() {
        ...
        ilog("picoio-blocklog - ${desc} took ${t} msec", ("desc", _desc)("t", duration));
    }
...
void blocklog::read_log() {
   ...
   PICO_ASSERT( end, block_log_exception, "No blocks found in block log" );
   PICO_ASSERT( end->block_num() > 1, block_log_exception, "Only one block found in block log" );
...
void smoke_test(bfs::path block_dir) {
   ...
   PICO_ASSERT( status == 0, block_log_exception, "cannot seek to ${file} ${pos} from beginning of file", ("file", td.block_file_name.string())("pos", sizeof(uint64_t)) );
   uint64_t file_pos;
   auto size = fread((void*)&file_pos, sizeof(uint64_t), 1, td.blk_in);
   PICO_ASSERT( size == 1, block_log_exception, "${file} read fails", ("file", td.block_file_name.string()) );
   status = fseek(td.blk_in, file_pos + trim_data::blknum_offset, SEEK_SET);
   PICO_ASSERT( status == 0, block_log_exception, "cannot seek to ${file} ${pos} from beginning of file", ("file", td.block_file_name.string())("pos", file_pos + trim_data::blknum_offset) );
   uint32_t bnum;
   ...
   PICO_ASSERT( td.last_block == bnum, block_log_exception, "blocks.log says last block is ${lb} which disagrees with blocks.index", ("lb", bnum) );
...
int main(int argc, char** argv) {
   ...
   options_description cli ("picoio-blocklog command line options");
   try {
      blocklog blog;
