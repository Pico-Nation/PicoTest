...
[[info | JSON input]]
| This command involves specifying JSON input which depends on underlying class definitions. Therefore, such JSON input is subject to change in future versions of the PICOIO software.
...
`--add-code` [code] add 'picoio.code' permission to specified permission authority

`--remove-code` [code] remove 'picoio.code' permission from specified permission authority
...
```sh
clpico set account permission test active '{"threshold":1,"keys":[{"key":"PICO8X7Mp7apQWtL6T2sfSZzBcQNUqZB7tARFEm9gA9Tn9nbMdsvBB","weight":1}],"accounts":[{"permission":{"actor":"acc2","permission":"active"},"weight":50}]}' owner
```
...
```sh
clpico set account permission test active '{"threshold":1,"keys":[],"accounts":[{"permission":{"actor":"acc1","permission":"active"},"weight":50},{"permission":{"actor":"sandwich","permission":"active"},"weight":1}]}' owner
```

The third example demonstrates how to set up permissions for multisig.

```sh
clpico set account permission test active '{"threshold" : 100, "keys" : [{"key":"PICO8X7Mp7apQWtL6T2sfSZzBcQNUqZB7tARFEm9gA9Tn9nbMdsvBB","weight":25}], "accounts" : [{"permission":{"actor":"sandwich","permission":"active"},"weight":75}]}' owner
```
...
```json
{
  ...
  "keys"            : [],     /*An array made up of individual permissions defined with an PICO PUBLIC KEY*/
  "accounts"        : []      /*An array made up of individual permissions defined with an PICO ACCOUNT*/
}
```
...
{
  "permission" : {
    "key"           : "PICO8X7Mp7apQWtL6T2sfSZzBcQNUqZB7tARFEm9gA9Tn9nbMdsvBB",
    "permission"    : "active"
  },
