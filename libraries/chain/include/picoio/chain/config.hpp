...
namespace picoio { namespace chain { namespace config {
...
const static name system_account_name    { N(picoio) };
const static name null_account_name      { N(picoio.null) };
const static name producers_account_name { N(picoio.prods) };
...
const static name picoio_auth_scope       { N(picoio.auth) };
const static name picoio_all_scope        { N(picoio.all) };

const static name active_name     { N(active) };
const static name owner_name      { N(owner) };
const static name picoio_any_name  { N(picoio.any) };
const static name picoio_code_name { N(picoio.code) };
...
const static picoio::chain::wasm_interface::vm_type default_wasm_runtime = picoio::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace picoio::chain::config

constexpr uint64_t PICO_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / picoio::chain::config::percent_100;
}

template<typename Number>
Number PICO_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + picoio::chain::config::percent_100 - picoio::chain::config::percent_1)  / picoio::chain::config::percent_100;
}
