...
#include <picoio/chain/webassembly/wavm.hpp>
...
#else
#define _REGISTER_PICOVMOC_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)
#endif
#include <picoio/chain/webassembly/runtime_interface.hpp>
...
#else
#define _REGISTER_PICO_VM_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)
#endif

using namespace fc;
using namespace picoio::chain::webassembly;
using namespace IR;
...
namespace picoio { namespace chain {
   ...
   namespace picovmoc { struct config; }
   ...
   struct wasm_interface_impl {
...
#ifdef PICOIO_PICO_VM_OC_RUNTIME_ENABLED
      struct picovmoc_tier {
         picovmoc_tier(const boost::filesystem::path& d, const picovmoc::config& c, const chainbase::database& db) : cc(d, c, db), exec(cc) {}
         picovmoc::code_cache_async cc;
         picovmoc::executor exec;
         picovmoc::memory mem;
      };
#endif

      wasm_interface_impl(wasm_interface::vm_type vm, bool picovmoc_tierup, const chainbase::database& d, const boost::filesystem::path data_dir, const picovmoc::config& picovmoc_config) : db(d), wasm_runtime_time(vm) {
         ...
         if(vm == wasm_interface::vm_type::pico_vm)
            runtime_interface = std::make_unique<webassembly::pico_vm_runtime::pico_vm_runtime<picoio::vm::interpreter>>();
#endif
#ifdef PICOIO_PICO_VM_JIT_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::pico_vm_jit)
            runtime_interface = std::make_unique<webassembly::pico_vm_runtime::pico_vm_runtime<picoio::vm::jit>>();
#endif
#ifdef PICOIO_PICO_VM_OC_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::pico_vm_oc)
            runtime_interface = std::make_unique<webassembly::picovmoc::picovmoc_runtime>(data_dir, picovmoc_config, d);
#endif
         if(!runtime_interface)
            PICO_THROW(wasm_exception, "${r} wasm runtime not supported on this platform and/or configuration", ("r", vm));

#ifdef PICOIO_PICO_VM_OC_RUNTIME_ENABLED
         if(picovmoc_tierup) {
            PICO_ASSERT(vm != wasm_interface::vm_type::pico_vm_oc, wasm_exception, "You can't use PICO VM OC as the base runtime when tier up is activated");
            picovmoc.emplace(data_dir, picovmoc_config, d);
         }
#endif
      }
      ...
      std::vector<uint8_t> parse_initial_memory(const Module& module) {
         ...
         for(const DataSegment& data_segment : module.dataSegments) {
            PICO_ASSERT(data_segment.baseOffset.type == InitializerExpression::Type::i32_const, wasm_exception, "");
            PICO_ASSERT(module.memories.defs.size(), wasm_exception, "");
...
#ifdef PICOIO_PICO_VM_OC_RUNTIME_ENABLED
         if(picovmoc) for(auto it = first_it; it != last_it; it++)
            picovmoc->cc.free_code(it->code_hash, it->vm_version);
#endif
         ...
         if(!it->module) {
            ...
            if (runtime_interface->inject_module(module)) {
               try {
                  ...
                  PICO_ASSERT(false, wasm_serialization_error,
                             e.message.c_str());
               } catch (const IR::ValidationException& e) {
                  PICO_ASSERT(false, wasm_serialization_error,
                             e.message.c_str());
...
#ifdef PICOIO_PICO_VM_OC_RUNTIME_ENABLED
      fc::optional<picovmoc_tier> picovmoc;
#endif
   };
...
#define _REGISTER_INTRINSIC_EXPLICIT(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   ...
   _REGISTER_PICO_VM_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)       \
   _REGISTER_PICOVMOC_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)
...
#define _REGISTER_INJECTED_INTRINSIC(R, CLS, INFO)\
   BOOST_PP_CAT(BOOST_PP_OVERLOAD(_REGISTER_INTRINSIC, _UNWRAP_SEQ INFO) _EXPAND_ARGS(CLS, PICOIO_INJECTED_MODULE_NAME, INFO), BOOST_PP_EMPTY())
