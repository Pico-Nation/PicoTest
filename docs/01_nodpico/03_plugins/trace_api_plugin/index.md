...
```console
# config.ini
plugin = picoio::trace_api_plugin
[options]
```
```sh
# command-line
nodpico ... --plugin picoio::trace_api_plugin [options]
```
...
```console
Config Options for picoio::trace_api_plugin:
...
Here is a `nodpico` configuration example for the `trace_api_plugin` when tracing some PICOIO reference contracts:

```sh
nodpico --data-dir data_dir --config-dir config_dir --trace-dir traces_dir
--plugin picoio::trace_api_plugin 
--trace-rpc-abi=picoio=abis/picoio.abi 
--trace-rpc-abi=picoio.token=abis/picoio.token.abi 
--trace-rpc-abi=picoio.msig=abis/picoio.msig.abi 
--trace-rpc-abi=picoio.wrap=abis/picoio.wrap.abi
```
...
The `trace-dir` option defines the directory on the filesystem where the trace log files are stored by the `trace_api_plugin`. These files are stable once the LIB block has progressed past a given slice and then can be deleted at any time to reclaim filesystem space. The deployed PICOIO system will tolerate any out-of-process management system that removes some or all of these files in this directory regardless of what data they represent, or whether there is a running `nodpico` instance accessing them or not.  Data which would nominally be available, but is no longer so due to manual maintenance, will result in a HTTP 404 response from the appropriate API endpoint(s).
