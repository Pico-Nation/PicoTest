...
SCRIPT_LOCATION="scripts/picoio_build.sh"
TEST_LABEL="[picoio_build]"

###################################################################
# ON MAC YOU NEED TO FULLY UNINSTALL PICOIO BEFORE THESE WILL PASS #
###################################################################
...
@test "${TEST_LABEL} > Testing Arguments & Options" {
    ...
    if [[ $NAME =~ "Amazon Linux" ]] || [[ $NAME == "CentOS Linux" ]]; then
        ...
        [[ ! -z $(echo "${output}" | grep "PICOIO compiler checks require the 'which'") ]] || exit
    fi
    ...
    run bash -c "./picoio_build.sh -y -P"
    ...
    # lack of -i
    [[ ! -z $(echo "${output}" | grep "PICOIO_INSTALL_DIR: ${HOME}/picoio/${PICOIO_VERSION}") ]] || exit
    ## -o
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -o Debug -P"
    [[ ! -z $(echo "${output}" | grep "CMAKE_BUILD_TYPE: Debug") ]] || exit
    ## -s
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -s PICO2 -P"
    [[ ! -z $(echo "${output}" | grep "CORE_SYMBOL_NAME: PICO2") ]] || exit
    ## -b
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -b /test -P"
    [[ ! -z $(echo "${output}" | grep "BOOST_LOCATION: /test") ]] || exit
    ## -i
    run bash -c "printf \"y\ny\nn\nn\n\"| ./$SCRIPT_LOCATION -i /NEWPATH -P"
    [[ ! -z $(echo "${output}" | grep "PICOIO_INSTALL_DIR: /NEWPATH") ]] || exit
    [[ ! -z $(echo "${output}" | grep "TEMP_DIR: ${HOME}/tmp") ]] || exit
        ...
        [[ ! -z $(echo "${output}" | grep "PICOIO_INSTALL_DIR: $TEMP_DIR/NEWPATH") ]] || exit
