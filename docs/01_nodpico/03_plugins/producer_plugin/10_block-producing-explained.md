...
peer validation for similar hardware/picoio-version/config will be <= m
...
#       * image will be rendered by gatsby-remark-graphviz plugin in picoio docs.
...
Running wasm-runtime=pico-vm-jit pico-vm-oc-enable on relay node will reduce the validation time.
