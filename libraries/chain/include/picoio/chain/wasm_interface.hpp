      ...
      public:
         ...
         { try {
            ...
            if( whitelisted_intrinsics != nullptr ) {
               ...
               PICO_ASSERT( mod_name == "env", wasm_exception,
            ...
            PICO_THROW( wasm_exception, "${module}.${export} unresolveable",
                      ("module",mod_name)("export",export_name) );
            return false;
   ...
   class wasm_interface {
      public:
         ...
         static std::string vm_type_string(vm_type vmtype) {
             switch (vmtype) {
             case vm_type::pico_vm:
                return "pico-vm";
             case vm_type::pico_vm_jit:
                return "pico-vm-jit";
             case vm_type::pico_vm_oc:
                return "pico-vm-oc";
             default:
                return "wabt";
             }
         ...
         wasm_interface(vm_type vm, bool picovmoc_tierup, const chainbase::database& d, const boost::filesystem::path data_dir, const picovmoc::config& picovmoc_config);
         ~wasm_interface();

         //call before dtor to skip what can be minutes of dtor overhead with some runtimes; can cause leaks
         void indicate_shutting_down();

         //validates code -- does a WASM validation pass and checks the wasm against PICOIO specific constraints
      ...
      private:
         ...
         friend class picoio::chain::webassembly::common::intrinsics_accessor;
   };
...
FC_REFLECT_ENUM( picoio::chain::wasm_interface::vm_type, (wabt)(pico_vm)(pico_vm_jit)(pico_vm_oc) )
