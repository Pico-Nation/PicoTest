#include <picoio/history_plugin/history_plugin.hpp>
#include <picoio/history_plugin/account_control_history_object.hpp>
#include <picoio/history_plugin/public_key_history_object.hpp>
...
CHAINBASE_SET_INDEX_TYPE(picoio::account_history_object, picoio::account_history_index)
CHAINBASE_SET_INDEX_TYPE(picoio::action_history_object, picoio::action_history_index)
...
namespace picoio {
      ...
      try {
         if( options.count( "filter-on" )) {
            ...
            for( auto& s : fo ) {
               if( s == "*" || s == "\"*\"" ) {
                  ...
                  wlog( "--filter-on * enabled. This can fill shared_mem, causing nodpico to stop." );
                  break;
               }
               std::vector<std::string> v;
               boost::split( v, s, boost::is_any_of( ":" ));
               PICO_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --filter-on", ("s", s));
               filter_entry fe{picoio::chain::name(v[0]), picoio::chain::name(v[1]), picoio::chain::name(v[2])};
               PICO_ASSERT( fe.receiver.to_uint64_t(), fc::invalid_arg_exception,
         ...
         if( options.count( "filter-out" )) {
            ...
            for( auto& s : fo ) {
               std::vector<std::string> v;
               boost::split( v, s, boost::is_any_of( ":" ));
               PICO_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --filter-out", ("s", s));
               filter_entry fe{picoio::chain::name(v[0]), picoio::chain::name(v[1]), picoio::chain::name(v[2])};
               PICO_ASSERT( fe.receiver.to_uint64_t(), fc::invalid_arg_exception,
   ...
   namespace history_apis {
      read_only::get_actions_result read_only::get_actions( const read_only::get_actions_params& params )const {
        ...
        PICO_ASSERT( end >= start, chain::plugin_exception, "end position is earlier than start position" );
      ...
      read_only::get_transaction_result read_only::get_transaction( const read_only::get_transaction_params& p )const {
         ...
         } PICO_RETHROW_EXCEPTIONS(transaction_id_type_exception, "Invalid transaction ID: ${transaction_id}", ("transaction_id", p.id))
         ...
         if( !in_history && !p.block_num_hint ) {
            PICO_THROW(tx_not_found, "Transaction ${id} not found in history and no block hint was given", ("id",p.id));
         }
         ...
         if( in_history ) {
            ...
            if (!found) {
               PICO_THROW(tx_not_found, "Transaction ${id} not found in history or in block number ${n}", ("id",p.id)("n", *p.block_num_hint));
            }
         }
