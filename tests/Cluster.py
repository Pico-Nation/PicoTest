...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="picoio-ignition-wd/bootlog.txt"

    # pylint: disable=too-many-arguments
    # walletd [True|False] Is kpicod running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="PICOtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet kpicod running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: pico server host
        port: pico server port
        walletHost: pico wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures pico mongo plugin
        ...
        self.picoioAccount=self.defProducerAccounts["picoio"]= Account("picoio")
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, sharedProducers=0, extraNodpicoArgs=" --http-max-response-time-ms 990000 ", useBiosBootFile=True, specificExtraNodpicoArgs=None, onlySetProds=False,
        ...
        extraNodpicoArgs: string of arguments to pass through to each nodoes instance (via --nodpico flag on launcher)
        ...
        specificExtraNodpicoArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-nodpico flags on launcher), example: { "5" : "--plugin picoio::test_control_api_plugin" }
        ...
        loadSystemContract: indicate whether the picoio.system contract should be loaded (setting this to False causes useBiosBootFile to be treated as False)
        """
        ...
        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.PicoServerName))
            return True
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodpico default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        cmd="%s -p %s -n %s -d %s -i %s -f %s --unstarted-nodes %s" % (
            Utils.PicoLauncherPath, pnodes, totalNodes, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
            producerFlag, unstartedNodes)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        nodpicoArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on \"*\" --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodpicoArgs += " --plugin picoio::wallet_api_plugin"
        if self.enableMongo:
            nodpicoArgs += " --plugin picoio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNodpicoArgs is not None:
            assert(isinstance(extraNodpicoArgs, str))
            nodpicoArgs += extraNodpicoArgs
        if Utils.Debug:
            nodpicoArgs += " --contracts-console"
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            nodpicoArgs += " --plugin picoio::producer_api_plugin"

        if nodpicoArgs:
            cmdArr.append("--nodpico")
            cmdArr.append(nodpicoArgs)

        if specificExtraNodpicoArgs is not None:
            assert(isinstance(specificExtraNodpicoArgs, dict))
            for nodeNum,arg in specificExtraNodpicoArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-nodpico")
                cmdArr.append(arg)
        ...
        if associatedNodeLabels is not None:
            for nodeNum,label in associatedNodeLabels.items():
                ...
                cmdArr.append(str(nodeNum))
                cmdArr.append("--spcfc-inst-nodpico")
                cmdArr.append(path)
        ...
        if nodes is None or startedNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.PicoServerName, startedNodes, len(nodes)))
            return False
        ...
        self.picoioAccount=self.defProducerAccounts["picoio"]
    ...
    def getClientVersion(verbose=False):
        ...
        try:
            cmd="%s version client" % (Utils.PicoClientPath)
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key --to-console" % (Utils.PicoClientPath)
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on pico server port %d" % (
        ...
        for i in range(0, count):
            ...
            for _ in range(0, count):
                #Utils.Print("nextPicoIdx: %d, n: %d" % (nextPicoIdx, n))
                nextPicoIdx=(nextPicoIdx + 1)%count
                if not self.nodes[nextPicoIdx].killed:
                    #Utils.Print("nextPicoIdx: %d" % (nextPicoIdx))
            ...
            #Utils.Print("nextPicoIdx: %d, count: %d" % (nextPicoIdx, count))
            node=self.nodes[nextPicoIdx]
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if Utils.Debug: Utils.Print("Validate funds on %s server port %d." %
                                        (Utils.PicoServerName, node.port))

            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on pico node port: %d" % (node.port))
                return False
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        initialBalances=self.nodes[0].getPicoBalances([self.defproduceraAccount] + self.accounts)
        assert(initialBalances)
        assert(isinstance(initialBalances, dict))
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.picoioAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse picoio private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by picoio-launcher."""
        ...
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        env = {
            "BIOS_CONTRACT_PATH": "unittests/contracts/old_versions/v1.6.0-rc3/picoio.bios",
            "BIOS_CURRENCY_SYMBOL": CORE_SYMBOL,
            "FEATURE_DIGESTS": ""
        }
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            env["BIOS_CONTRACT_PATH"] = "unittests/contracts/old_versions/v1.7.0-develop-preactivate_feature/picoio.bios"
        ...
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull, env=env):
            if not silent: Utils.Print("Launcher failed to shut down pico cluster.")
            return None
        ...
        picoioName="picoio"
        picoioKeys=producerKeys[picoioName]
        picoioAccount=Account(picoioName)
        picoioAccount.ownerPrivateKey=picoioKeys["private"]
        picoioAccount.ownerPublicKey=picoioKeys["public"]
        picoioAccount.activePrivateKey=picoioKeys["private"]
        picoioAccount.activePublicKey=picoioKeys["public"]
        producerKeys.pop(picoioName)

        if not self.walletMgr.importKey(picoioAccount, ignWallet):
            Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (picoioName))
            return None
        ...
        for name, keys in producerKeys.items():
            data="{\"from\":\"picoio\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init picoio transfer")
            opts="--permission picoio@active"
            if name != "picoio":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from picoio.token to %s." % (name))
                    return None
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        ...
        trans=biosNode.publishContract(picoioAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, picoioAccount, 0)
            if trans is None:
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission picoio@active"
                    myTrans=biosNode.pushMessage("picoio", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("picoio", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but picoio).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "picoio"
        ...
        picoioTokenAccount=copy.deepcopy(picoioAccount)
        picoioTokenAccount.name="picoio.token"
        trans=biosNode.createAccount(picoioTokenAccount, picoioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (picoioTokenAccount.name))
            return None

        picoioRamAccount=copy.deepcopy(picoioAccount)
        picoioRamAccount.name="picoio.ram"
        trans=biosNode.createAccount(picoioRamAccount, picoioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (picoioRamAccount.name))
            return None

        picoioRamfeeAccount=copy.deepcopy(picoioAccount)
        picoioRamfeeAccount.name="picoio.ramfee"
        trans=biosNode.createAccount(picoioRamfeeAccount, picoioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (picoioRamfeeAccount.name))
            return None

        picoioStakeAccount=copy.deepcopy(picoioAccount)
        picoioStakeAccount.name="picoio.stake"
        trans=biosNode.createAccount(picoioStakeAccount, picoioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (picoioStakeAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(picoioTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=picoioTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\"}" % (picoioAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to picoio contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (picoioAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (picoioAccount.name)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to picoio contract.")
            return None
        ...
        Utils.Print("Verify picoio issue, Expected: %s" % (expectedAmount))
        actualAmount=biosNode.getAccountPicoBalanceStr(picoioAccount.name)
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (picoioAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (picoioAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (picoioTokenAccount.name, name))
                return None
        ...
        if loadSystemContract:
            ...
            opts="--permission %s@active" % (picoioAccount.name)
            trans=biosNode.pushMessage(picoioAccount.name, action, data, opts)
    ...
    @staticmethod
    def pgrepPicoServers(timeout=None):
        cmd=Utils.pgrepCmd(Utils.PicoServerName)
    ...
    @staticmethod
    def pgrepPicoServerPattern(nodeInstance):
        dataLocation=Utils.getNodeDataDir(nodeInstance)
        return r"[\n]?(\d+) (.* --data-dir %s .*)\n" % (dataLocation)

    # Populates list of PicoInstanceInfo objects, matched to actual running instances
    def discoverLocalNodes(self, totalNodes, timeout=None):
        nodes=[]

        psOut=Cluster.pgrepPicoServers(timeout)
        if psOut is None:
    ...
    def discoverLocalNode(self, nodeNum, psOut=None, timeout=None):
        ...
        if psOut is None:
            Utils.Print("ERROR: No nodes discovered.")
            return None
        pattern=Cluster.pgrepPicoServerPattern(nodeNum)
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.PicoServerName, pattern))
            return None
    ...
    def discoverBiosNode(self, timeout=None):
        psOut=Cluster.pgrepPicoServers(timeout=timeout)
        pattern=Cluster.pgrepPicoServerPattern("bios")
        Utils.Print("pattern={\n%s\n}, psOut=\n%s\n" % (pattern,psOut))
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.PicoServerName, pattern))
    ...
    # Kills a percentange of Pico instances starting from the tail and update picoInstanceInfos state
    def killSomePicoInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        killSignal=signal.SIGKILL
        if killSignalStr == Utils.SigTermTag:
            killSignal=signal.SIGTERM
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.PicoServerName, killSignal))
    ...
    def relaunchPicoInstances(self, cachePopen=False):
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster nodpico instances. allInstances will kill all nodpico instances running on the system."""
        cmd="%s -k 9" % (Utils.PicoLauncherPath)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down pico cluster.")

        if allInstances:
            # ocassionally the launcher cannot kill the pico server
            cmd="pkill -9 %s" % (Utils.PicoServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down pico cluster.")
    ...
    def bounce(self, nodes, silent=True):
        """Bounces nodpico instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down nodpico instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
