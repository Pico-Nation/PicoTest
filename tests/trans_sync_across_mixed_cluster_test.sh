...
killAll()
{
  programs/picoio-launcher/picoio-launcher -k 15
}

cleanup()
{
    rm -rf etc/picoio/node_*
    rm -rf var/lib/node_*
}
...
getHeadBlockNum()
{
  INFO="$(programs/clpico/clpico get info)"
  verifyErrorCode "clpico get info"
...
# stand up nodpico cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --nodpico \"--plugin picoio::wallet_api_plugin\" $launcherOpts
programs/picoio-launcher/picoio-launcher --nodpico "--plugin picoio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/clpico/clpico --port $port2 get block 1)"
    verifyErrorCode "clpico get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/clpico/clpico create key)"
verifyErrorCode "clpico create key"
...
PASSWORD_INITA="$(programs/clpico/clpico wallet create --name inita)"
verifyErrorCode "clpico wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/clpico/clpico wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "clpico wallet import"
programs/clpico/clpico wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "clpico wallet import"
programs/clpico/clpico wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "clpico wallet import"
programs/clpico/clpico wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "clpico wallet import"
...
ACCOUNT_INFO="$(programs/clpico/clpico create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "clpico create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/clpico/clpico get account testera)"
verifyErrorCode "clpico get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/clpico/clpico transfer inita testera 975321 "test transfer")"
    verifyErrorCode "clpico transfer"
   ...
   TRANS_INFO="$(programs/clpico/clpico --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "clpico get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
