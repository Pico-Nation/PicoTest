---
content_title: Nodpico Plugins
---

## Overview

Plugins extend the core functionality implemented in `nodpico`. Some plugins are mandatory, such as `chain_plugin`, `net_plugin`, and `producer_plugin`, which reflect the modular design of `nodpico`. The other plugins are optional as they provide nice to have features, but non-essential for the nodes operation.
...
[[info | Nodpico is modular]]
| Plugins add incremental functionality to `nodpico`. Unlike runtime plugins, `nodpico` plugins are built at compile-time.
