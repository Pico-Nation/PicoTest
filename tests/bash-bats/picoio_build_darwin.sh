...
export SCRIPT_LOCATION="scripts/picoio_build.sh"
export TEST_LABEL="[picoio_build_darwin]"
...
## Needed to load picoio_build_ files properly; it can be empty
@test "${TEST_LABEL} > General" {
    ...
    [[ ! -z $(echo "${output}" | grep "Starting PICOIO Dependency Install") ]] || exit
    ...
    [[ ! -z $(echo "${output}" | grep "brew tap picoio/picoio") ]] || exit
    ...
    [[ ! -z $(echo "${output}" | grep "Starting PICOIO Build") ]] || exit
    [[ ! -z $(echo "${output}" | grep " --with-iostreams --with-date_time") ]] || exit # BOOST
    [[ ! -z $(echo "${output}" | grep "PICOIO has been successfully built") ]] || exit
}