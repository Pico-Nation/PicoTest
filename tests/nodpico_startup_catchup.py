...
try:
    ...
    for nodeNum in range(txnGenNodeNum, txnGenNodeNum+startedNonProdNodes):
        specificExtraNodpicoArgs[nodeNum]="--plugin picoio::txn_test_gen_plugin --txn-test-gen-account-prefix txntestacct"
    ...
    if cluster.launch(prodCount=prodCount, onlyBios=False, pnodes=pnodes, totalNodes=totalNodes, totalProducers=pnodes*prodCount,
                      useBiosBootFile=False, specificExtraNodpicoArgs=specificExtraNodpicoArgs, unstartedNodes=catchupCount, loadSystemContract=False) is False:
    ...
    txnGenNodes[0].txnGenCreateTestAccounts(cluster.picoioAccount.name, cluster.picoioAccount.activePrivateKey)
