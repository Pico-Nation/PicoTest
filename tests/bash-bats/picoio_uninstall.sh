...
SCRIPT_LOCATION="scripts/picoio_uninstall.sh"
TEST_LABEL="[picoio_uninstall]"
...
@test "${TEST_LABEL} > Testing user prompts" {
  ...
  [[ $output =~ " - PICOIO Removal Complete" ]] || exit
  ## First no shows "Cancelled..."
  run bash -c "echo \"n\" | ./$SCRIPT_LOCATION"
  [[ "${output##*$'\n'}" =~ "Cancelled PICOIO Removal!" ]] || exit
...
@test "${TEST_LABEL} > -i" {
  run ./$SCRIPT_LOCATION -y -i picoiotest
  [[ $output =~ .*/picoiotest ]] || exit
  ([[ ! $output =~ "Library/Application\ Support/picoio" ]] && [[ ! $output =~ ".local/share/picoio" ]]) || exit
  [[ ! $output =~ "PICOIO Removal Complete" ]] || exit
}
