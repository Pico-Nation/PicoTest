...
public:
   ...
   fc::variant get_global_state() {
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(picoio.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create picoio.msig and picoio.token
        create_accounts({N(picoio.msig), N(picoio.token), N(picoio.ram), N(picoio.ramfee), N(picoio.stake), N(picoio.vpay), N(picoio.bpay), N(picoio.saving) });
        // Set code for the following accounts:
        //  - picoio (code: picoio.bios) (already set by tester constructor)
        //  - picoio.msig (code: picoio.msig)
        //  - picoio.token (code: picoio.token)
        // set_code_abi(N(picoio.msig), contracts::picoio_msig_wasm(), contracts::picoio_msig_abi().data());//, &picoio_active_pk);
        // set_code_abi(N(picoio.token), contracts::picoio_token_wasm(), contracts::picoio_token_abi().data()); //, &picoio_active_pk);

        set_code_abi(N(picoio.msig),
                     contracts::picoio_msig_wasm(),
                     contracts::picoio_msig_abi().data());//, &picoio_active_pk);
        set_code_abi(N(picoio.token),
                     contracts::picoio_token_wasm(),
                     contracts::picoio_token_abi().data()); //, &picoio_active_pk);

        // Set privileged for picoio.msig and picoio.token
        set_privileged(N(picoio.msig));
        set_privileged(N(picoio.token));

        // Verify picoio.msig and picoio.token is privileged
        const auto& picoio_msig_acc = get<account_metadata_object, by_name>(N(picoio.msig));
        BOOST_TEST(picoio_msig_acc.is_privileged() == true);
        const auto& picoio_token_acc = get<account_metadata_object, by_name>(N(picoio.token));
        BOOST_TEST(picoio_token_acc.is_privileged() == true);


        // Create SYS tokens in picoio.token, set its manager as picoio
        ...
        create_currency(N(picoio.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to picoio.system
        issue(N(picoio.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(picoio.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == name("picoio"));
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), picoio_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), picoio_assert_message_exception);
