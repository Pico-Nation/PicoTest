---
content_title: picoio-blocklog
link_text: picoio-blocklog
---

`picoio-blocklog` is a command-line interface (CLI) utility that allows node operators to perform low-level tasks on the block logs created by a `nodpico` instance. `picoio-blocklog` can perform one of the following operations:
...
When `picoio-blocklog` is launched, the utility attempts to perform the specified operation, then yields the following possible outcomes:
