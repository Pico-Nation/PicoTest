...
The `chain_plugin` is a core plugin required to process and aggregate chain data on an PICOIO node.
...
```console
Config Options for picoio::chain_plugin:
  ...
  --wasm-runtime pico-vm|pico-vm-jit      Override default WASM runtime (wabt)
  --pico-vm-oc-enable                    Enable optimized compilation in WASM
