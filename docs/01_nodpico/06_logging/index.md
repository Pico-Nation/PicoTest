---
content_title: Nodpico Logging
---

Logging for `nodpico` is controlled by the `logging.json` file. CLI options can be passed to `nodpico` to [setup `logging.json`](00_setup-logging.json.md). The logging configuration file can be used to define [appenders](#appenders) and tie them to [loggers](#loggers) and [logging levels](01_logging-levels.md).

## Appenders

The logging library built into PICOIO supports two appender types:
...
The logging library built into PICOIO currently supports the following loggers:
