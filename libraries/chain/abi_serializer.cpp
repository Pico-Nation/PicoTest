...
namespace picoio { namespace chain {
   ...
   void abi_serializer::set_abi(const abi_def& abi, const yield_function_t& yield) {
      ...
      PICO_ASSERT(starts_with(abi.version, "picoio::abi/1."), unsupported_abi_version_exception, "ABI has an unsupported version");
      ...
      for( const auto& td : abi.types ) {
         PICO_ASSERT(!_is_type(td.new_type_name, ctx), duplicate_abi_type_def_exception,
       ...
       *  an invalid ABI
       */
      PICO_ASSERT( typedefs.size() == abi.types.size(), duplicate_abi_type_def_exception, "duplicate type definition detected" );
      PICO_ASSERT( structs.size() == abi.structs.size(), duplicate_abi_struct_def_exception, "duplicate struct definition detected" );
      PICO_ASSERT( actions.size() == abi.actions.size(), duplicate_abi_action_def_exception, "duplicate action definition detected" );
      PICO_ASSERT( tables.size() == abi.tables.size(), duplicate_abi_table_def_exception, "duplicate table definition detected" );
      PICO_ASSERT( error_messages.size() == abi.error_messages.size(), duplicate_abi_err_msg_def_exception, "duplicate error message definition detected" );
      PICO_ASSERT( variants.size() == abi.variants.value.size(), duplicate_abi_variant_def_exception, "duplicate variant definition detected" );
   ...
   int abi_serializer::get_integer_size(const std::string_view& type) const {
      PICO_ASSERT( is_integer(type), invalid_type_inside_abi, "${type} is not an integer type", ("type",impl::limit_size(type)));
   ...
   const struct_def& abi_serializer::get_struct(const std::string_view& type)const {
      ...
      PICO_ASSERT( itr != structs.end(), invalid_type_inside_abi, "Unknown struct ${type}", ("type",impl::limit_size(type)) );
      return itr->second;
   }
   ...
   void abi_serializer::validate( impl::abi_traverse_context& ctx )const {
      for( const auto& t : typedefs ) { try {
         ...
         while( itr != typedefs.end() ) {
            ctx.check_deadline();
            PICO_ASSERT( find(types_seen.begin(), types_seen.end(), itr->second) == types_seen.end(), abi_circular_def_exception,
      ...
      for( const auto& t : typedefs ) { try {
         PICO_ASSERT(_is_type(t.second, ctx), invalid_type_inside_abi, "${type}", ("type",impl::limit_size(t.second)) );
      } FC_CAPTURE_AND_RETHROW( (t) ) }
      for( const auto& s : structs ) { try {
         if( s.second.base != type_name() ) {
            ...
            while( current->base != type_name() ) {
               ...
               PICO_ASSERT( find(types_seen.begin(), types_seen.end(), base.name) == types_seen.end(), abi_circular_def_exception,
         ...
         for( const auto& field : s.second.fields ) { try {
            ctx.check_deadline();
            PICO_ASSERT(_is_type(_remove_bin_extension(field.type), ctx), invalid_type_inside_abi,
      ...
      for( const auto& s : variants ) { try {
         for( const auto& type : s.second.types ) { try {
            ctx.check_deadline();
            PICO_ASSERT(_is_type(type, ctx), invalid_type_inside_abi, "${type}", ("type",impl::limit_size(type)) );
         } FC_CAPTURE_AND_RETHROW( (type) ) }
      } FC_CAPTURE_AND_RETHROW( (s) ) }
      for( const auto& a : actions ) { try {
        ctx.check_deadline();
        PICO_ASSERT(_is_type(a.second, ctx), invalid_type_inside_abi, "${type}", ("type",impl::limit_size(a.second)) );
                                            ...
                                            fc::mutable_variant_object& obj, impl::binary_to_variant_context& ctx )const
   {
      ...
      PICO_ASSERT( s_itr != structs.end(), invalid_type_inside_abi, "Unknown type ${type}", ("type",ctx.maybe_shorten(type)) );
      ...
      for( uint32_t i = 0; i < st.fields.size(); ++i ) {
         ...
         if( !stream.remaining() ) {
            ...
            if( encountered_extension ) {
               PICO_THROW( abi_exception, "Encountered field '${f}' without binary extension designation while processing struct '${p}'",
                          ("f", ctx.maybe_shorten(field.name))("p", ctx.get_path_string()) );
            }
            PICO_THROW( unpack_exception, "Stream unexpectedly ended; unable to unpack field '${f}' of struct '${p}'",
                       ("f", ctx.maybe_shorten(field.name))("p", ctx.get_path_string()) );
                                                   ...
                                                   impl::binary_to_variant_context& ctx )const
   {
      ...
      if( btype != built_in_types.end() ) {
         ...
         } PICO_RETHROW_EXCEPTIONS( unpack_exception, "Unable to unpack ${class} type '${type}' while processing '${p}'",
      ...
      if ( is_array(rtype) ) {
         ...
         } PICO_RETHROW_EXCEPTIONS( unpack_exception, "Unable to unpack size of array '${p}'", ("p", ctx.get_path_string()) )
         ...
         for( decltype(size.value) i = 0; i < size; ++i ) {
            ...
            PICO_ASSERT( !v.is_null(), unpack_exception, "Invalid packed array '${p}'", ("p", ctx.get_path_string()) );
            vars.emplace_back(std::move(v));
         }
         // QUESTION: Why would the assert below ever fail?
         PICO_ASSERT( vars.size() == size.value,
                     unpack_exception,
         ...
         } PICO_RETHROW_EXCEPTIONS( unpack_exception, "Unable to unpack presence flag of optional '${p}'", ("p", ctx.get_path_string()) )
         ...
         if( v_itr != variants.end() ) {
            ...
            } PICO_RETHROW_EXCEPTIONS( unpack_exception, "Unable to unpack tag of variant '${p}'", ("p", ctx.get_path_string()) )
            PICO_ASSERT( (size_t)select < v_itr->second.types.size(), unpack_exception,
      ...
      PICO_ASSERT( mvo.size() > 0, unpack_exception, "Unable to unpack '${p}' from stream", ("p", ctx.get_path_string()) );
   ...
   { try {
      ...
      if( btype != built_in_types.end() ) {
         ...
         ctx.hint_variant_type_if_in_array( v_itr );
         auto& v = v_itr->second;
         PICO_ASSERT( var.is_array() && var.size() == 2, pack_exception,
                    "Expected input to be an array of two items while processing variant '${p}'", ("p", ctx.get_path_string()) );
         PICO_ASSERT( var[size_t(0)].is_string(), pack_exception,
         ...
         PICO_ASSERT( it != v.types.end(), pack_exception,
         ...
         if( var.is_object() ) {
            ...
            for( uint32_t i = 0; i < st.fields.size(); ++i ) {
               ...
               if( vo.contains( string(field.name).c_str() ) ) {
                  if( disallow_additional_fields )
                     PICO_THROW( pack_exception, "Unexpected field '${f}' found in input object while processing struct '${p}'",
                                ("f", ctx.maybe_shorten(field.name))("p", ctx.get_path_string()) );
                  {
                     auto h1 = ctx.push_to_path( impl::field_path_item{ .parent_struct_itr = s_itr, .field_ordinal = i } );
                  ...
                  PICO_THROW( pack_exception, "Missing field '${f}' in input object while processing struct '${p}'",
                             ("f", ctx.maybe_shorten(field.name))("p", ctx.get_path_string()) );
               }
            }
         } else if( var.is_array() ) {
            const auto& va = var.get_array();
            PICO_ASSERT( st.base == type_name(), invalid_type_inside_abi,
            ...
            for( uint32_t i = 0; i < st.fields.size(); ++i ) {
               ...
               if( va.size() > i ) {
                  ...
                  PICO_THROW( pack_exception, "Early end to input array specifying the fields of struct '${p}'; require input for field '${f}'",
                             ("p", ctx.get_path_string())("f", ctx.maybe_shorten(field.name)) );
               }
            }
         } else {
            PICO_THROW( pack_exception, "Unexpected input encountered while processing struct '${p}'", ("p",ctx.get_path_string()) );
         }
      } else {
         PICO_THROW( invalid_type_inside_abi, "Unknown type ${type}", ("type",ctx.maybe_shorten(type)) );
      }
   ...
   namespace impl {
      ...
      fc::scoped_exit<std::function<void()>> abi_traverse_context_with_path::push_to_path( const path_item& item ) {
         std::function<void()> callback = [this](){
            PICO_ASSERT( path.size() > 0, abi_exception,
      ...
      void abi_traverse_context_with_path::set_array_index_of_path_back( uint32_t i ) {
         PICO_ASSERT( path.size() > 0, abi_exception, "path is empty" );

         auto& b = path.back();

         PICO_ASSERT( b.contains<array_index_path_item>(), abi_exception, "trying to set array index without first pushing new array index item" );
