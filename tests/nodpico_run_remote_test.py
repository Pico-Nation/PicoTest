...
###############################################################
# nodpico_run_remote_test
#
#  Tests remote capability of the nodpico_run_test. Test will setup cluster and pass nodes info to nodpico_run_test. E.g.
#  nodpico_run_remote_test.py -v --clean-run --dump-error-detail
#
###############################################################
...
pnodes=1
total_nodes=pnodes
actualTest="tests/nodpico_run_test.py"
...
try:
    ...
    Print("Starting up %s test: %s" % ("nodpico", actualTest))
    Print("cmd: %s\n" % (cmd))
    ...
    TestHelper.shutdown(cluster, None, testSuccessful, killPicoInstances, False, False, killAll, dumpErrorDetails)
