...
proxy::proxy( picoio::name self, picoio::name first_receiver, picoio::datastream<const char*> ds )
...
void proxy::on_transfer( name from, name to, asset quantity, const std::string& memo ) {
   ...
   if( from == self ) {
      ...
      transaction out;
      picoio::token::transfer_action a( "picoio.token"_n, {self, "active"_n} );
...
void proxy::on_error( uint128_t sender_id, picoio::ignore<std::vector<char>> ) {
