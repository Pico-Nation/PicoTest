#!/bin/bash
# The purpose of this test is to ensure that the output of the "nodpico --version" command matches the version string defined by our CMake files
# If the environment variable BUILDKITE_TAG is empty or unset, this test will echo success
echo '##### Nodpico Version Label Test #####'
...
# orient ourselves
[[ "$PICOIO_ROOT" == '' ]] && PICOIO_ROOT=$(echo $(pwd)/ | grep -ioe '.*/pico/')
[[ "$PICOIO_ROOT" == '' ]] && PICOIO_ROOT=$(echo $(pwd)/ | grep -ioe '.*/PICOIO/picoio/')
[[ "$PICOIO_ROOT" == '' ]] && PICOIO_ROOT=$(echo $(pwd)/ | grep -ioe '.*/build/' | sed 's,/build/,,')
echo "Using PICOIO_ROOT=\"$PICOIO_ROOT\"."
# determine expected value
CMAKE_CACHE="$PICOIO_ROOT/build/CMakeCache.txt"
CMAKE_LISTS="$PICOIO_ROOT/CMakeLists.txt"
if [[ -f "$CMAKE_CACHE" && $(cat "$CMAKE_CACHE" | grep -c 'DOXY_PICO_VERSION') > 0 ]]; then
    echo "Parsing \"$CMAKE_CACHE\"..."
    EXPECTED="v$(cat "$CMAKE_CACHE" | grep 'DOXY_PICO_VERSION' | cut -d '=' -f 2)"
...
if [[ "$EXPECTED" == '' ]]; then
    ...
    echo ''
    echo '$ cat "$CMAKE_CACHE" | grep "DOXY_PICO_VERSION"'
    cat "$CMAKE_CACHE" | grep "DOXY_PICO_VERSION"
    echo '$ pwd'
    pwd
    echo '$ ls -la "$PICOIO_ROOT"'
    ls -la "$PICOIO_ROOT"
    echo '$ ls -la "$PICOIO_ROOT/build"'
    ls -la "$PICOIO_ROOT/build"
    exit 1
fi
echo "Expecting \"$EXPECTED\"..."
# get nodpico version
ACTUAL=$($PICOIO_ROOT/build/bin/nodpico --version) || : # nodpico currently returns -1 for --version
# test
