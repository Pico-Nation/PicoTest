#include <picoio/chain/webassembly/pico-vm-oc/executor.hpp>
#include <picoio/chain/webassembly/pico-vm-oc/code_cache.hpp>
#include <picoio/chain/webassembly/pico-vm-oc/memory.hpp>
#include <picoio/chain/webassembly/pico-vm-oc/intrinsic_mapping.hpp>
...
#error PICO VM OC is not compatible with Clang ShadowCallStack
#endif
#endif
...
static void segv_handler(int sig, siginfo_t* info, void* ctx)  {
   ...
   if((uintptr_t)info->si_addr >= cb_in_main_segment->execution_thread_code_start &&
      (uintptr_t)info->si_addr < cb_in_main_segment->execution_thread_code_start+cb_in_main_segment->execution_thread_code_length)
         siglongjmp(*cb_in_main_segment->jmp, PICOVMOC_EXIT_CHECKTIME_FAIL);
   ...
   if((uintptr_t)info->si_addr >= cb_in_main_segment->execution_thread_memory_start &&
      (uintptr_t)info->si_addr < cb_in_main_segment->execution_thread_memory_start+cb_in_main_segment->execution_thread_memory_length)
         siglongjmp(*cb_in_main_segment->jmp, PICOVMOC_EXIT_SEGV);
...
static intrinsic grow_memory_intrinsic PICOVMOC_INTRINSIC_INIT_PRIORITY("picovmoc_internal.grow_memory", IR::FunctionType::get(IR::ResultType::i32,{IR::ValueType::i32,IR::ValueType::i32}),
  (void*)&pico_vm_oc_grow_memory,
  boost::hana::index_if(intrinsic_table, ::boost::hana::equal.to(BOOST_HANA_STRING("picovmoc_internal.grow_memory"))).value()
);

//This is effectively overriding the picoio_exit intrinsic in wasm_interface
static void picoio_exit(int32_t code) {
   siglongjmp(*pico_vm_oc_get_jmp_buf(), PICOVMOC_EXIT_CLEAN_EXIT);
   __builtin_unreachable();
}
static intrinsic picoio_exit_intrinsic("env.picoio_exit", IR::FunctionType::get(IR::ResultType::none,{IR::ValueType::i32}), (void*)&picoio_exit,
  boost::hana::index_if(intrinsic_table, ::boost::hana::equal.to(BOOST_HANA_STRING("env.picoio_exit"))).value()
);

static void throw_internal_exception(const char* const s) {
   *reinterpret_cast<std::exception_ptr*>(pico_vm_oc_get_exception_ptr()) = std::make_exception_ptr(wasm_execution_error(FC_LOG_MESSAGE(error, s)));
   siglongjmp(*pico_vm_oc_get_jmp_buf(), PICOVMOC_EXIT_EXCEPTION);
   __builtin_unreachable();
}

#define DEFINE_PICOVMOC_TRAP_INTRINSIC(module,name) \
	void name(); \
	static intrinsic name##Function PICOVMOC_INTRINSIC_INIT_PRIORITY(#module "." #name,IR::FunctionType::get(),(void*)&name, \
     boost::hana::index_if(intrinsic_table, ::boost::hana::equal.to(BOOST_HANA_STRING(#module "." #name))).value() \
   ); \
	void name()

DEFINE_PICOVMOC_TRAP_INTRINSIC(picovmoc_internal,depth_assert) {
   throw_internal_exception("Exceeded call depth maximum");
}

DEFINE_PICOVMOC_TRAP_INTRINSIC(picovmoc_internal,div0_or_overflow) {
   throw_internal_exception("Division by 0 or integer overflow trapped");
}

DEFINE_PICOVMOC_TRAP_INTRINSIC(picovmoc_internal,indirect_call_mismatch) {
   throw_internal_exception("Indirect call function type mismatch");
}

DEFINE_PICOVMOC_TRAP_INTRINSIC(picovmoc_internal,indirect_call_oob) {
   throw_internal_exception("Indirect call index out of bounds");
}

DEFINE_PICOVMOC_TRAP_INTRINSIC(picovmoc_internal,unreachable) {
...
executor::executor(const code_cache_base& cc) {
   ...
   if(arch_prctl(ARCH_GET_GS, &current_gs) || current_gs)
      wlog("x86_64 GS register is not set as expected. PICO VM OC may not run correctly on this platform");
...
void executor::execute(const code_descriptor& code, const memory& mem, apply_context& context) {
   ...
   cb->current_call_depth_remaining = picoio::chain::wasm_constraints::maximum_call_depth+2;
   ...
   switch(sigsetjmp(*cb->jmp, 0)) {
      case 0:
         ...
         break;
      //case 1: clean picoio_exit
      case PICOVMOC_EXIT_CHECKTIME_FAIL:
         context.trx_context.checktime();
         break;
      case PICOVMOC_EXIT_SEGV:
         PICO_ASSERT(false, wasm_execution_error, "access violation");
         break;
      case PICOVMOC_EXIT_EXCEPTION: //exception
