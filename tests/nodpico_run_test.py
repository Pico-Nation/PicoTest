...
# nodpico_run_test
#
# General test that tests a wide range of general use actions around nodpico and kpicod
#
###############################################################
...
try:
    ...
    if not dontLaunch:
        walletAccounts.append(cluster.picoioAccount)
    ...
    contractDir="unittests/contracts/picoio.token"
    wasmFile="picoio.token.wasm"
    abiFile="picoio.token.abi"
    ...
    if not enableMongo:
        ...
        account=node.getPicoAccountFromDb(currencyAccount.name)
        ...
        if abiName != "account" or abiActionName != "close" or abiType != "close":
            errorExit("FAILURE - get PICO account failed", raw=True)
    ...
    account=node.getPicoAccount(defproduceraAccount.name, exitOnError=True)
    ...
    if not enableMongo:
        ...
        rawAccount=node.getPicoAccount(defproduceraAccount.name, exitOnError=True, returnType=ReturnType.raw)
