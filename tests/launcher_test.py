...
# launcher-test
#
# Specifically tests using the bios bootstrap script that is created by picoio-launcher
#
###############################################################
...
try:
    ...
    if not dontLaunch:
        ...
        killPicoInstances=False

        print("Stand up walletd")
        if walletMgr.launch() is False:
            cmdError("%s" % (WalletdName))
            errorExit("Failed to stand up pico walletd.")
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.picoioAccount,cluster.defproduceraAccount])
    ...
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.picoioAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.picoioAccount, stakedDeposit=0, waitForTransBlock=False, exitOnError=True)

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.picoioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.picoioAccount, buyRAM=1000000, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.picoioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.picoioAccount, buyRAM=1000000, waitForTransBlock=True, exitOnError=True)
    ...
    actualAmount=node.getAccountPicoBalanceStr(testeraAccount.name)
    ...
    Print("Verify transfer, Expected: %s" % (expectedAmount))
    actualAmount=node.getAccountPicoBalanceStr(currencyAccount.name)
    if expectedAmount != actualAmount:
        cmdError("FAILURE - transfer failed")
        errorExit("Transfer verification failed. Excepted %s, actual: %s" % (expectedAmount, actualAmount))
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful, killPicoInstances, killWallet, keepLogs, killAll, dumpErrorDetails)
