...
class currency_tester : public TESTER {
   public:
      ...
      asset get_balance(const account_name& account) const {
         return get_currency_balance(N(picoio.token), symbol(SY(4,CUR)), account);
      }
      ...
      auto issue(const account_name& to, const std::string& quantity, const std::string& memo = "") {
         auto trace = push_action(N(picoio.token), N(issue), mutable_variant_object()
                                  ("to",       to)
                                  ("quantity", quantity)
                                  ("memo",     memo)
                                  );
      ...
      currency_tester()
         :TESTER(),abi_ser(json::from_string(contracts::picoio_token_abi().data()).as<abi_def>(), abi_serializer::create_yield_function( abi_serializer_max_time ))
      {
         create_account( N(picoio.token));
         set_code( N(picoio.token), contracts::picoio_token_wasm() );

         auto result = push_action(N(picoio.token), N(create), mutable_variant_object()
                 ("issuer",       picoio_token)
         ...
         result = push_action(N(picoio.token), N(issue), mutable_variant_object()
                 ("to",       picoio_token)
      ...
      static const name picoio_token;
};

const name currency_tester::picoio_token = N(picoio.token);
...
BOOST_AUTO_TEST_CASE( bootstrap ) try {
   ...
   currency_tester t;
   auto actual = t.get_currency_balance(N(picoio.token), expected.get_symbol(), N(picoio.token));
...
BOOST_FIXTURE_TEST_CASE( test_transfer, currency_tester ) try {
   ...
   {
      auto trace = push_action(N(picoio.token), N(transfer), mutable_variant_object()
         ("from", picoio_token)
         ("to",   "alice")
...
BOOST_FIXTURE_TEST_CASE( test_duplicate_transfer, currency_tester ) {
   ...
   BOOST_REQUIRE_THROW(push_action(N(picoio.token), N(transfer), mutable_variant_object()
                                    ("from", picoio_token)
                                    ("to",   "alice")
                                    ("quantity", "100.0000 CUR")
                                    ("memo", "fund Alice")),
...
BOOST_FIXTURE_TEST_CASE( test_overspend, currency_tester ) try {
   ...
   // Overspend!
   {
      ...
      BOOST_CHECK_EXCEPTION( push_action(N(alice), N(transfer), data),
                             picoio_assert_message_exception, picoio_assert_message_is("overdrawn balance") );
      produce_block();
...
BOOST_FIXTURE_TEST_CASE( test_input_quantity, currency_tester ) try {
   ...
   {
      auto trace = transfer(picoio_token, N(alice), "100.0000 CUR");
   ...
   {
      BOOST_REQUIRE_THROW(transfer(N(alice), N(carl), "20.50 USD"), picoio_assert_message_exception);
   }
